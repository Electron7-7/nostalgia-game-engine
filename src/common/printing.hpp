...
template<class... Args>
    bool print_error(std::format_string<Args...> fmt, Args&&... args)
...
template<class... Args>
    ...
    { std::println("\x1b[1;36m[DEBUG] {}\x1b[0m", std::format(fmt, std::forward<Args>(args)...)); }
...
template<class... Args>
    ...
    { std::println("\x1b[1;35m[APP DEBUG] {}\x1b[0m", std::format(fmt, std::forward<Args>(args)...)); }

template<class... Args>
    void print_jolt_debug(std::format_string<Args...> fmt, Args&&... args)
    {   std::println("\x1b[1;32m[JOLT] {}\x1b[0m", std::format(fmt, std::forward<Args>(args)...)); }
...
#   define print_jolt_debug(FMT,ARGS...) {}
#endif // DEBUGGING
