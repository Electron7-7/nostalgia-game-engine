...
void ImGui_Debugger::InspectTheatreWindow()
{
    ...
    if(BeginChild("Theatre Inspector", {500, 500}, ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeY | ImGuiChildFlags_ResizeX))
    {
        ...
        if(CollapsingHeader("Thing Selection", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            auto uids{g_pTheatreManager->Current()->ThingIDs()};
        ...
        if(auto thinker{DCast<Thinker>(selected.ptr)})
        {
            ...
            BeginChild("Parent", {}, ImGuiChildFlags_AutoResizeY);
                ...
                if(auto parent_ptr{g_pTheatreManager->Current()->GetThinker(selected.parent)};
            ...
            else if(auto actor{DCast<Actor3D>(selected.ptr)})
            {
                ...
                else if(auto camera3d{DCast<Camera3D>(selected.ptr)})
                {
                    ...
                    auto cur_vp{g_pTheatreManager->Current()->GetThinker<Viewport>(camera3d->ViewportID())};
            ...
            else if(auto actor{DCast<Actor2D>(selected.ptr)})
            {
                ...
                else if(auto camera2d{DCast<Camera2D>(selected.ptr)})
                {
                    ...
                    auto cur_vp{g_pTheatreManager->Current()->GetThinker<Viewport>(camera2d->ViewportID())};
                    TextF("Viewport: {} [{}]", cur_vp->name(), cur_vp->uid()());
                    SameLine();
                    if(Button("Inspect"))
                    {
            ...
            BeginChild("Children", {}, ImGuiChildFlags_AutoResizeY);
                ...
                if(Button("+"))
                {
                    print_error_enum(g_pTheatreManager->Current()
                ...
                for(FAUTO child : children)
                {
                    ...
                    {
                        auto thing{g_pTheatreManager->Current()->GetThing(child)};
        ...
        else if(auto resource{DCast<Resource>(selected.ptr)})
        {
            if(auto texture{DCast<ImageTexture>(selected.ptr)})
            {
                ...
                {
                    {SAMPLER_FILTER_NEAREST, EnumRegistry::GetEnumName(SAMPLER_FILTER_NEAREST).data()},
                    {SAMPLER_FILTER_LINEAR,  EnumRegistry::GetEnumName(SAMPLER_FILTER_LINEAR).data()},
                    {SAMPLER_FILTER_NONE,    EnumRegistry::GetEnumName(SAMPLER_FILTER_NONE).data()},
                };
