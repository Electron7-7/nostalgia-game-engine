...
static const std::map<FileSystem::OverwriteAction, const char*>
action_type{
    { FileSystem::OVERWRITE, "Overwrite"},
...
void ImGui_Editor::TheatreLoadingWindow()
{
    BeginChild("Theatre Debugging");
        ...
        BeginDisabled(not m_sTheatreRunning);
            ...
            if(BeginCombo("Duplicate Filename Action", action_type.at(m_sCurrentOverwriteAction), ImGuiComboFlags_WidthFitPreview))
            {
                for(FAUTO [action, name] : action_type)
                {
                    const bool _selected{action == m_sCurrentOverwriteAction};
                    if(Selectable(name))
                        { m_sCurrentOverwriteAction = action; }
                    if(_selected)
                        { SetItemDefaultFocus(); }
                }
                EndCombo();
