...
const char* __get_print_message(Error error)
{
    ...
    Case(ERR_FILE_EXISTS, "a file exists at a given path")
    Case(ERR_DIR_EXISTS, "a directory exists at a given path")
    Case(ERR_FILE_LOAD, "`FileData::LoadFile` error")
    Case(ERR_FILE_OPEN, "file open error")
    ...
    Case(ERR_INVALID_PATH, "invalid file path error")
    ...
    Case(ERR_MAX, "some number is already as large as it's allowed to be")
    Case(ERR_MIN, "some number is already as small as it's allowed to be")
